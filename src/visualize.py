...
def visualize_history(history, count, error_only=True):
    ...
    test, = plot_field(x, test_error, 'Error',
                       'Percent Error', label='Test set error')
...
def show_races(df, iv='ViolentCrimesPerPop'):
    race_pcts = ['racepctblack', 'racePctWhite', 'racePctHisp', 'racePctAsian']
    colors = ['black', 'gray', 'red', 'yellow']
    for race_pct, col in zip(race_pcts, colors):
        plt.plot(df[race_pct], df[iv], '.', color=col)
        plt.show()
